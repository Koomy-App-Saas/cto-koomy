-- KOOMY PROD_V2 SCHEMA PATCH (IDEMPOTENT) - 2026-01-28
-- Safe to re-run
...
DO $$ BEGIN CREATE TYPE billing_status AS ENUM ('trialing', 'active', 'past_due', 'canceled', 'unpaid'); EXCEPTION WHEN duplicate_object THEN NULL; END $$;
...
-- CRITICAL: Communities columns for server boot
...
DO $$ BEGIN IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE tablename = 'users' AND indexname = 'users_firebase_uid_key') THEN CREATE UNIQUE INDEX users_firebase_uid_key ON users(firebase_uid) WHERE firebase_uid IS NOT NULL; END IF; END $$;
...
DO $$ BEGIN IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE tablename = 'accounts' AND indexname = 'accounts_email_key') THEN ALTER TABLE accounts ADD CONSTRAINT accounts_email_key UNIQUE (email); END IF; EXCEPTION WHEN duplicate_object THEN NULL; END $$;
...
DO $$ BEGIN IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE tablename = 'admin_invitations' AND indexname = 'admin_invitations_token_key') THEN ALTER TABLE admin_invitations ADD CONSTRAINT admin_invitations_token_key UNIQUE (token); END IF; EXCEPTION WHEN duplicate_object THEN NULL; END $$;
...
DO $$ BEGIN IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE tablename = 'platform_sessions' AND indexname = 'platform_sessions_token_key') THEN ALTER TABLE platform_sessions ADD CONSTRAINT platform_sessions_token_key UNIQUE (token); END IF; EXCEPTION WHEN duplicate_object THEN NULL; END $$;
...
DO $$ BEGIN IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE tablename = 'platform_verification_tokens' AND indexname = 'platform_verification_tokens_token_key') THEN ALTER TABLE platform_verification_tokens ADD CONSTRAINT platform_verification_tokens_token_key UNIQUE (token); END IF; EXCEPTION WHEN duplicate_object THEN NULL; END $$;

-- Unique indexes on communities
DO $$ BEGIN IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE tablename = 'communities' AND indexname = 'communities_custom_domain_key') THEN CREATE UNIQUE INDEX communities_custom_domain_key ON communities(custom_domain) WHERE custom_domain IS NOT NULL; END IF; END $$;
DO $$ BEGIN IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE tablename = 'communities' AND indexname = 'communities_self_enrollment_slug_key') THEN CREATE UNIQUE INDEX communities_self_enrollment_slug_key ON communities(self_enrollment_slug) WHERE self_enrollment_slug IS NOT NULL; END IF; END $$;
DO $$ BEGIN IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE tablename = 'communities' AND indexname = 'communities_member_join_code_key') THEN CREATE UNIQUE INDEX communities_member_join_code_key ON communities(member_join_code) WHERE member_join_code IS NOT NULL; END IF; END $$;
