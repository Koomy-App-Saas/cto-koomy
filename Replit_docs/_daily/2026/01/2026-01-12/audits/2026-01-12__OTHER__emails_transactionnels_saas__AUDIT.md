...
> Date: Janvier 2026  
> Scope: Emails envoyés DE Koomy VERS les clients SaaS (admin/owner des communautés)
...
1. **Nouveaux EmailTypes** dans `emailTypes.ts`
2. **Templates email** dans `template.ts`
3. **Fonctions send\*** dans `sendBrandedEmail.ts`
4. **Wiring** dans `stripe.ts` (webhooks) et daily job
5. **Daily job scheduler** pour emails temporels (J+7, J+25, J+50)
6. **Recording** dans `subscription_emails_sent` pour éviter doublons
...
### Recommandation
1. **Phase 1 (P0)** : Implémenter E04, E08, E10, E11 — Bloque le risque support/juridique
2. **Phase 2 (P1)** : Implémenter E01, E06, E07, E09 — Améliore l'UX et la rétention
3. **Phase 3 (P2)** : Implémenter E02, E03, E05, E12, E13 — Polish final
